    ...
    def install_ampf_command():
        """
        Guides the user through setting up the AMPF service configuration.
        This function is designed to be called directly by the CLI.
        """
        print("\n--- AMPF Service Setup ---")
    ...
    def main():
        ...
        parser = argparse.ArgumentParser(description="AMPF CLI - Service Management")
        parser.add_argument("command", help="Command to execute (e.g., 'install')")

        args = parser.parse_args()

        if args.command == "install":
            install_ampf_command()
        else:
    ...
    if __name__ == "__main__":
        ...
        try:
            ...
            sys.exit(1) # Use sys.exit for clean exit in CLI
        main()
